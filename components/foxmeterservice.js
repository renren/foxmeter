/*
	FoxMeter - An HTTP analyzer addon for Firefox
...
const CLASS_ID = Components.ID("{09b6f1bc-128d-407a-8434-01def2269b63}");

// description
const CLASS_NAME = "FoxMeter Service";

// textual unique identifier
const CONTRACT_ID = "@xiaonei.com/foxmeter;1";
...
FoxMeterService.prototype = 
{
	...
	// All requests (holds FoxMeterRequest objects)
	Requests: null,
	...
	init: function() 
	{
		...
		this.Observer = new FoxMeterObserver(this);
		
		this.Preferences = new FoxMeterPreferences();
	...
	addController: function(FoxMeterControllerReference)
	{
		this.Controllers.push(FoxMeterControllerReference);
		FoxMeterControllerReference.ControllerIndex = this.Controllers.length;
	},
	
	removeController: function(FoxMeterControllerReference)
	{
		for (var i = 0; i < this.Controllers.length; i++)
		{
			if (this.Controllers[i] === FoxMeterControllerReference)
			{
	...
	{
		// a new request
		var request = new FoxMeterRequest(requestEvent.FoxMeter, requestEvent.HttpChannel, requestEvent.Context, requestEvent);
	...
	handleRequestEvent: function(requestEvent)
	{
		...
		if (!this.Preferences.ShowFoxMeterHelperRequests && requestEvent.HttpChannel.owner) 
		{
			try 
			{
				if (requestEvent.HttpChannel.owner.QueryInterface(Components.interfaces.nsISupportsString).data == "FoxMeterResponseLoaderFlagger") 
				{
					// don't log.
					return;
				}
		...
		{
			case this.FoxMeterEventSourceType.ON_MODIFY_REQUEST:
			case this.FoxMeterEventSourceType.ON_EXAMINE_RESPONSE:
			case this.FoxMeterEventSourceType.ON_EXAMINE_MERGED_RESPONSE:
			case this.FoxMeterEventSourceType.EVENTSINK_ON_PROGRESS:
			case this.FoxMeterEventSourceType.EVENTSINK_ON_STATUS:
			case this.FoxMeterEventSourceType.WEBPROGRESS_ON_STATUS_CHANGED:
			case this.FoxMeterEventSourceType.WEBPROGRESS_ON_STATE_CHANGED:
			case this.FoxMeterEventSourceType.WEBPROGRESS_ON_SECURITY_CHANGED:
			case this.FoxMeterEventSourceType.WEBPROGRESS_ON_PROGRESS_CHANGED:
			case this.FoxMeterEventSourceType.WEBPROGRESS_ON_LOCATION_CHANGED:
				//find matching request 
				var index = this.getPendingRequestForRequestEvent(requestEvent);
				if (index == -1)
				{
					if (requestEvent.EventSource != this.FoxMeterEventSourceType.ON_MODIFY_REQUEST) 
					{
	...
	getStatusTextFromCode: function(FoxMeterStatusCode, status)
	{
		var statusText = "";
		switch(FoxMeterStatusCode)
		{
			case this.FoxMeterStatusCodeType.SOCKETTRANSPORT:
				switch(status)
				{
			...
			case this.FoxMeterStatusCodeType.WEBPROGRESS_TRANSITION:
			...
			case this.FoxMeterStatusCodeType.WEBPROGRESS_TYPE:
			...
			case this.FoxMeterStatusCodeType.WEBPROGRESS_MODIFIER:	
			...
			case this.FoxMeterStatusCodeType.WEBPROGRESS_SECURITY:
			...
			case this.FoxMeterStatusCodeType.WEBPROGRESS_SECURITY_STRENGTH:
			...
			case this.FoxMeterStatusCodeType.LOADFLAGS_CHANNEL:
			...
			case this.FoxMeterStatusCodeType.LOADFLAGS_REQUEST:
			...
			case this.FoxMeterStatusCodeType.LOADFLAGS_CACHING:
	...
	getEventSourceName: function(EventSource)
	{
		switch(EventSource)
		{
			...
			case this.FoxMeterEventSourceType.SCANNED_COMPLETE:
...
FoxMeterService.prototype.FoxMeterEventSourceType = 
{
	ON_MODIFY_REQUEST: 0,
...
FoxMeterService.prototype.FoxMeterStatusCodeType =
{
	SOCKETTRANSPORT: 0,
...
function FoxMeterPreferences() 
{
	this.init();
};

FoxMeterPreferences.prototype = 
{
	prefs: null,
	...
	_ShowFoxMeterHelperRequests: null,
	...
	init: function() 
	{
		...
		this._ShowFoxMeterHelperRequests = this.prefs.getBoolPref("ShowFoxMeterHelperRequests");
	...
	observe: function(subject, topic, data)
	{
		...
		switch(data)
		{
			...
			case "ShowFoxMeterHelperRequests":
				this._ShowFoxMeterHelperRequests = this.prefs.getBoolPref("ShowFoxMeterHelperRequests");
				break;
	...
	get ShowFoxMeterHelperRequests() 
	{ 
		return this._ShowFoxMeterHelperRequests;
	},
	set ShowFoxMeterHelperRequests(value) 
	{
		this._ShowFoxMeterHelperRequests = value;
		this.prefs.setBoolPref("ShowFoxMeterHelperRequests", value);
	},
...
// FoxMeterRequest
function FoxMeterRequest(FoxMeterServiceReference, HttpChannelReference, FoxMeterContext, FoxMeterRequestEventReference)
{
	try 
	{
	...
	this.FoxMeter = FoxMeterServiceReference;
	this.Context = FoxMeterContext;
	
	this.init(FoxMeterRequestEventReference);
}
FoxMeterRequest.prototype = 
{
	FoxMeter: null,
	HttpChannel: null,
	Context: null,
	...
	init: function(requestEvent)
	{
		...
		// store event sink
		this.FoxMeterRequestEventSink = requestEvent.FoxMeterRequestEventSink;
	...
	{
		// aborted:		
		if (this.EventSource == this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_STATE_CHANGED
			//&& getStatusTextFromCode(FoxMeterStatusCodeType.WEBPROGRESS_TRANSITION, this.EventSourceData["flags"]) == "STATE_STOP"
			&& this.EventSourceData["flags"] & Components.interfaces.nsIWebProgressListener.STATE_STOP
			&& this.EventSourceData["status"] == utils.FoxMeterNsResultErrors.NS_BINDING_ABORTED)
		{
			// aborted
			this.setAborted();
			return;
		...
		if (this.IsBackground && this.EventSource == this.FoxMeter.FoxMeterEventSourceType.ON_EXAMINE_RESPONSE)
		{
			this.setFinished();
			return;
		}
		
		if (this.EventSource == this.FoxMeter.FoxMeterEventSourceType.ON_EXAMINE_MERGED_RESPONSE)
		{
		...
		if (this.EventSource == this.FoxMeter.FoxMeterEventSourceType.ON_EXAMINE_RESPONSE
	...
	updateFromRequestEvent: function(requestEvent)
	{
		...
		if (requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.EVENTSINK_ON_STATUS
			|| requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_STATUS_CHANGED)
		{
			if ((this.IsLoadingBody || this.IsSending) 
				&& this.RequestLog[this.RequestLog.length - 1].EventSource == this.FoxMeter.FoxMeterEventSourceType.EVENTSINK_ON_STATUS
				//&& (getStatusTextFromCode(FoxMeterStatusCodeType.SOCKETTRANSPORT, requestEvent.EventSourceData["status"]) == "STATUS_RECEIVING_FROM"
				&& (requestEvent.EventSourceData["status"] == Components.interfaces.nsISocketTransport.STATUS_RECEIVING_FROM
					...
					//|| getStatusTextFromCode(FoxMeterStatusCodeType.SOCKETTRANSPORT, requestEvent.EventSourceData["status"]) == "STATUS_SENDING_TO"))
			{
		...
		if (requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.EVENTSINK_ON_PROGRESS
			|| requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_PROGRESS_CHANGED)
		{
			if (requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.EVENTSINK_ON_PROGRESS)
			{
			...
			else if (requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_PROGRESS_CHANGED)
			{
		...
		this.logEvent(new FoxMeterRequestLogData(requestEvent));
	...
	adjustDataFromRequestEvent: function(requestEvent)
	{
		...
		if (requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.ON_MODIFY_REQUEST)
		{
			// start sending
			this.IsSending = true;
		}
		
		if (requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.ON_EXAMINE_RESPONSE)
		{
			// start receiving
		...
		// cache info stuff
		if (requestEvent.EventSource == this.FoxMeter.FoxMeterEventSourceType.SCANNED_COMPLETE
	...
	showCacheInfo: function() 
	{
		...
		{
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "(none)", "(This request was not served from cache)");
			return;
		}
		...
		try 
		{
			...
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Key", CacheInfo.key);
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Expires", utils.formatDateTime(CacheInfo.expirationTime));
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Hit Count", CacheInfo.fetchCount);
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Last Hit", utils.formatDateTime(CacheInfo.lastFetched));
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Last Modification", utils.formatDateTime(CacheInfo.lastModified));
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Client ID", CacheInfo.clientID);
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Device ID", CacheInfo.deviceID);
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Size", CacheInfo.dataSize);
			
			if (this.CacheFile != null) 
			{
				var CacheFileInfo = this.CacheFile.QueryInterface(Components.interfaces.nsIFile);
				
				this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Filename", CacheFileInfo.leafName);
				this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Filepath", CacheFileInfo.target);
				this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Filesize", CacheFileInfo.fileSize);
				this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "File Last Modification", utils.formatDateTime(CacheFileInfo.lastModifiedTime / 1000));
			}
			else
			{
				this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "Filename", "n/a");
			}
		} 
		catch (ex)
		{
			this.FoxMeter.addHeaderRow("hf_CacheInfoChildren", "(error)", "(There was an error accessing the cache information)");
		}
	},
	...
	complete: function()
	{
		...
		if (this.Status == utils.FoxMeterNsResultErrors.NS_BINDING_ABORTED)
		//if (this.Status != 0)
		{
			// aborted
			this.setAborted();
		...
		this.updateFromRequestEvent(
			new FoxMeterRequestEvent(
				this.FoxMeter, 
				this.HttpChannel, 
				this.FoxMeter.FoxMeterEventSourceType.SCANNED_COMPLETE, 
				null, 
		...
		try {
			...
			if (this.HttpChannel.loadGroup && this.HttpChannel.loadGroup.groupObserver) {
				...
				try 
				{
					go.removeProgressListener(this.FoxMeter.Observer);
				}
				catch(ex) 
				{}
			}
			
			this.FoxMeterRequestEventSink.HttpChannel.notificationCallbacks = this.FoxMeterRequestEventSink.OriginalNotificationCallbacks;
			this.FoxMeterRequestEventSink = null;
	...
	isRedirect : function()
	{
		if (this.Status && this.Status == utils.FoxMeterNsResultErrors.NS_BINDING_REDIRECTED) 
		{
			return true;
		}
...
// FoxMeterRequestEvent
function FoxMeterRequestEvent(FoxMeterReference, HttpChannelReference, EventSourceType, EventSourceMiscData, FoxMeterContext)
{
	try 
	{
		this.HttpChannel = HttpChannelReference.QueryInterface(Components.interfaces.nsIHttpChannel);
	...
	this.FoxMeter = FoxMeterReference;
...
FoxMeterRequestEvent.prototype = 
{
	FoxMeter: null, // reference
	...
	init: function()
	{
		...
		if (this.EventSource == this.FoxMeter.FoxMeterEventSourceType.ON_MODIFY_REQUEST)
		{
			// Get Request Headers
			var dummyHeaderInfo = new FoxMeterHeaderInfo();
		...
		if (this.EventSource == this.FoxMeter.FoxMeterEventSourceType.EVENTSINK_ON_PROGRESS)
		{
			// update byte count
		...
		if (this.EventSource == this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_PROGRESS_CHANGED)
		{
			// update byte count
	...
	getPostData: function()
	{
		...
		try 
		{
			...
			if (postChannel.uploadStream) 
			{
				...
				var PostDataHandler = new FoxMeterPostDataHandler(this);
...
// FoxMeterRequestEventSink
function FoxMeterRequestEventSink(FoxMeterReference, HttpChannel)
{
	this.init(FoxMeterReference, HttpChannel);
}
FoxMeterRequestEventSink.prototype =
{
	// Properties
	FoxMeter: null,
	...
	// Constructor
	init: function(FoxMeterReference, HttpChannel) 
	{
		this.FoxMeter = FoxMeterReference;
	...
	*/
	onProgress: function(request, context, progress, progressMax)
	{
		...
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, request, this.FoxMeter.FoxMeterEventSourceType.EVENTSINK_ON_PROGRESS, eventSourceData, utils.getContextFromRequest(request)));
	...
	onStatus: function(request, context, status, statusArg)
	{
		...
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, request, this.FoxMeter.FoxMeterEventSourceType.EVENTSINK_ON_STATUS, eventSourceData, utils.getContextFromRequest(request)));
		// forward to possible other notificationCallbacks
		try {
			if (this.OriginalNotificationCallbacks != null) 
			{
...
// FoxMeterObserver
function FoxMeterObserver(FoxMeterReference)
{
	this.init(FoxMeterReference);
}
FoxMeterObserver.prototype =
{
	// Properties
	FoxMeter: null,
	
	// Constructor
	init: function(FoxMeterReference) 
	{
		this.FoxMeter = FoxMeterReference;
	},
	...
	{
		// force caching
		this.FoxMeter.forceCaching(HttpChannel);
		...
		try {
			...
			var event = new FoxMeterRequestEvent(
				this.FoxMeter, 
				HttpChannel, 
				this.FoxMeter.FoxMeterEventSourceType.ON_MODIFY_REQUEST, 
				eventSourceData, 
				utils.getContextFromRequest(HttpChannel));
			
			event.FoxMeterRequestEventSink = new FoxMeterRequestEventSink(this.FoxMeter, HttpChannel);
				
			this.FoxMeter.handleRequestEvent(event);
			}
	...
	{
		var eventSourceData = new Object();
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, HttpChannel, this.FoxMeter.FoxMeterEventSourceType.ON_EXAMINE_RESPONSE, eventSourceData, utils.getContextFromRequest(HttpChannel)));
	},
	
	onExamineMergedResponse: function(HttpChannel) 
	{
		var eventSourceData = new Object();
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, HttpChannel, this.FoxMeter.FoxMeterEventSourceType.ON_EXAMINE_MERGED_RESPONSE, eventSourceData, utils.getContextFromRequest(HttpChannel)));
	},
	...
	onStateChange: function(progress, request, flags, status)
	{
		...
		eventSourceData["status"] = status;
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, request, this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_STATE_CHANGED, eventSourceData, utils.getContextFromRequest(request)));
	},
	...
	onProgressChange: function(progress, request, curSelfProgress, maxSelfProgress, curTotalProgress, maxTotalProgress) 
	{
		...
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, request, this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_PROGRESS_CHANGED, eventSourceData, utils.getContextFromRequest(request)));
	},
	...
	onLocationChange: function(progress, request, uri) 
	{
		...
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, request, this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_LOCATION_CHANGED, eventSourceData, utils.getContextFromRequest(request)));
	},
	...
	onStatusChange: function(progress, request, status, message) 
	{
		...
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, request, this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_STATUS_CHANGED, eventSourceData, utils.getContextFromRequest(request)));
	},
	...
	onSecurityChange: function(progress, request, state) 
	{
		...
		this.FoxMeter.handleRequestEvent(new FoxMeterRequestEvent(this.FoxMeter, request, this.FoxMeter.FoxMeterEventSourceType.WEBPROGRESS_ON_SECURITY_CHANGED, eventSourceData, utils.getContextFromRequest(request)));
	},
	/********************************************/
	...
	* nsISupportsString
	*/
	data: "FoxMeterObserver",
	
	
	toString: function()
	{
		return "FoxMeterObserver";
	},
	/*********************************************/
...
function FoxMeterContext(win, browser, chrome, persistedState)
{
    this.windows = [];    
    this.panelMap = {};
    this.sidePanelNames = {};
    this.sourceCache = new FoxMeterSourceCache(win);
}
// ************************************************************************************************

// FoxMeterSourceCache
function FoxMeterSourceCache(win)
{
	this.charset = null;
	if (win != null) {
...
FoxMeterSourceCache.prototype =
{
    ...
    load: function(url, myPostData, ckey, request)
    {
        ...
        try
        {
			...
			channel.owner = new FoxMeterResponseLoaderFlagger();
        }
        catch(ex)
        {
            return;
        ...
        try
        {
        	var listener = new FoxMeterSourceCacheStreamListener(url, this, request, this.charset);
    ...
    loadAsync: function(url, cb)
    {
        ...
        var listener = new FoxMeterSourceCacheStreamListener(url, this, cb);
        channel.asyncOpen(listener, null);            
    } 
}
// ************************************************************************************************

// FoxMeterSourceCacheStreamListener
function FoxMeterSourceCacheStreamListener(url, cache, request, charset)
{
...
FoxMeterSourceCacheStreamListener.prototype =
{
...
// FoxMeterResponseLoaderFlagger
function FoxMeterResponseLoaderFlagger() 
{}
FoxMeterResponseLoaderFlagger.prototype =
{
	data: "FoxMeterResponseLoaderFlagger",
	
	toString: function()
	{
		return "FoxMeterResponseLoaderFlagger";
	},
...
// FoxMeterHeaderInfo
function FoxMeterHeaderInfo()
{
	this.init();
}
FoxMeterHeaderInfo.prototype = 
{
	Headers: null,
...
// FoxMeterPostDataHandler
// Contains code from LiveHeaders and TamperData
function FoxMeterPostDataHandler(hfRequest) 
{
...
FoxMeterPostDataHandler.prototype = 
{
	rewind: function() 
	{
...
// FoxMeterRequestLogData
function FoxMeterRequestLogData(request)
{
	this.init(request);
}
FoxMeterRequestLogData.prototype = 
{
	EventSource: null,
	EventSourceData: null,
	Timestamp: null,
...
var utils = {
	...
	getContextFromWindow: function(win)
	{
		if (win == null) 
		{
			return new FoxMeterContext(null, null, null, false);
		}
		else 
		{
			var browser = this.getBrowserByWindow(win);
			var chrome = browser ? browser.chrome : null;
			return new FoxMeterContext(win, browser, chrome, false);	
		}
	},
...
var FoxMeterServiceModule = 
{
	...
	getClassObject: function(aCompMgr, aCID, aIID)
	{
		...
		if (aCID.equals(CLASS_ID))
			return this.FoxMeterServiceFactory;
	...
	***********************************************************/
	FoxMeterServiceFactory:
	{
		createInstance: function(aOuter, aIID)
		{
			...
			return (new FoxMeterService()).QueryInterface(aIID);
		}
	}
};
...
if (typeof XPCOMUtils != "undefined")
{
	if (XPCOMUtils.generateNSGetFactory) {
		// FF 4+
		var NSGetFactory = XPCOMUtils.generateNSGetFactory([FoxMeterService]);	
	}
}

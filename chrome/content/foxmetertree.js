...
function FoxMeterTree(treeElement, FoxMeterControllerReference)
{
   this.init(treeElement, FoxMeterControllerReference);
}

FoxMeterTree.prototype = 
{
	FoxMeter: null,
	TreeElement: null,
	//treebox: null,
	...
	init: function(treeElement, FoxMeterControllerReference)
	{
		this.FoxMeter = FoxMeterControllerReference;
	...
	get rowCount() 
	{
		return this.FoxMeter.FilteredRequests.length;
		//return this.FoxMeter.Requests.length;
	},
	...
	{
		var request = this.FoxMeter.FilteredRequests[row];
			...
			if (column.id)
			{
	     	...
	     	switch(column)
	     	{
	     		case "hf_Column_Started":
	        		return com.xiaonei.utils.formatTime(new Date(request.StartTimestamp - this.FoxMeter.FoxMeterService.StartTime.getTime()));
				...
				case "hf_Column_Time":
					...
					return com.xiaonei.utils.formatTimeDifference(request.StartTimestamp, request.EndTimestamp);
				...
				case "hf_Column_Sent":
					...
					{
						rString = com.xiaonei.utils.humanizeSize(request.getBytesSent(), 6) + "/" + com.xiaonei.utils.humanizeSize(request.getBytesSentTotal(), 6);
					}
					else
					{
						rString = com.xiaonei.utils.humanizeSize(request.getBytesSentTotal(), 6);	
					}
				...
				case "hf_Column_Received":
					...
					if (!request.IsFinished)
					{
						...
						rString = com.xiaonei.utils.humanizeSize(request.getBytesLoaded(), 6) + "/" + com.xiaonei.utils.humanizeSize(request.getBytesLoadedTotal(), 6);
					}
					else
					{
						rString = com.xiaonei.utils.humanizeSize(request.getBytesLoaded(), 6);	
					}
				...
				case "hf_Column_Type":
					if (request.hasErrorCode())
					{
						...
						{
							return request.ContentType + " (" + com.xiaonei.utils.nsResultErrors[request.Status.toString(16)] + ")";
						}
						
						return com.xiaonei.utils.nsResultErrors[request.Status.toString(16)];
					}
	...
	getRowProperties: function(row, props) 
	{
		...
		if (!this.FoxMeter.FoxMeterService.Preferences.ColorRequests)
		{
			return;
		}
	...
	rowCountChanged: function(index, count) 
	{
		...
		if (this.treebox) 
		{
			...
			//if ((lvr + 1) >= index || this.FoxMeter.isAutoScroll())
			if (this.FoxMeter.isAutoScroll())
			{
	...
	getCurrent: function() 
	{
		if (this.FoxMeter.FilteredRequests[this.TreeElement.currentIndex]) 
		{
			return this.FoxMeter.FilteredRequests[this.TreeElement.currentIndex];
		} 
		else
		{
			return null;

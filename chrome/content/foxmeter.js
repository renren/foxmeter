...
// FoxMeterController
function FoxMeterController()
{
	this.init();
}
FoxMeterController.prototype =
{
	FoxMeterService: null,
	...
	init: function ()
	{
		...
		this.FoxMeterService = Components.classes["@xiaonei.com/foxmeter;1"].getService().wrappedJSObject;
		...
		this.stringBundle = this.StringBundleService.createBundle("chrome://foxmeter/locale/foxmeter.properties");
	...
	initGraphics: function (wmode)
	{
		...
		if (!this.FoxMeterService.Preferences.ShowDebugTab)
		{
			document.getElementById("hf_DebugTabHeader").collapsed = "true";
			document.getElementById("hf_DebugTabHeader").disabled = "true";
		}

		this.FoxMeterService.addController(this);
		this.initFilteredRequests(this.FoxMeterService.Requests);
		if (this.FoxMeterService.IsWatching)
		{
		...
		if (this.WindowMode)
		{
			this.FoxMeterService.FoxMeterWindow = this;
	...
	loadXMLPrettyPrintXSL: function ()
	{
		...
		var test = xslDoc.load("chrome://foxmeter/content/xmlprettyprint.xsl");
    ...
    cmd_hf_showSummary: function () 
    {
        window.open("chrome://foxmeter/content/summary.xul", "summary",
                  "resizable,scrollbars,chrome=yes,centerscreen=yes,width=600,height=300");
        return;
    },
	...
	{
		var FoxMeterPanel = document.getElementById("hf_PanelNormal");
		var FoxMeterPanelSplitter = document.getElementById("hf_PanelSplitter");
		...
		if (this.FoxMeterService.Preferences.AlwaysOpenDetached)
		{
			this.cmd_hf_detach();
	...
	{
		this.FilteredRequests = new Array();
		for (var i = 0; i < this.FoxMeterService.Requests.length; i++)
		{
			if (this.FoxMeterService.Requests[i].Url.indexOf(this.QuickFilterText) != -1)
			{
				// filter matched
				this.FilteredRequests.push(this.FoxMeterService.Requests[i]);
			}
		}
	...
	cmd_hf_startWatching: function ()
	{
		if (!this.FoxMeterService.IsWatching)
		{
			this.FoxMeterService.startWatching();
		}

		this.FoxMeterService.callControllerMethod("disableButton_startWatching");
	},
	...
	cmd_hf_stopWatching: function ()
	{
		if (this.FoxMeterService.IsWatching)
		{
			this.FoxMeterService.stopWatching();
		}

		this.FoxMeterService.callControllerMethod("disableButton_stopWatching");
	},
	...
	{
		this.FoxMeterService.clearRequests();

		this.FoxMeterService.callControllerMethod("clear");
	},
	...
	switchToDetachedWindowIfOpened: function ()
	{
		...
		// if already opened detached, switch to opened window
		if (this.FoxMeterService.FoxMeterWindow)
		{
			if (!FoxMeterPanel.collapsed)
			{
				// close open panel
				FoxMeterPanelSplitter.collapsed = FoxMeterPanel.collapsed = true;
			}

			// window already open. switch to that
			this.FoxMeterService.FoxMeterWindow.setFocus();
	...
	togglePanel: function ()
	{
		...
		// normal panel toggle
		FoxMeterPanelSplitter.collapsed = FoxMeterPanel.collapsed = (FoxMeterPanel.collapsed) ? false : true;

		// command checked
		document.getElementById("cmd_hf_togglePanel").setAttribute("checked", !FoxMeterPanelSplitter.collapsed);
	},
	...
	clearRequestTree: function ()
	{
		...
		if (treeElement)
		{
			this.RequestTree = new FoxMeterTree(treeElement, this);
		}
	},
	...
	selectionChange_RequestDetails: function ()
	{
		try
		{
			...
			if (this.isSelectedTab_Debug() && !this.FoxMeterService.Preferences.ShowDebugTab)
			{
	...
	selectionChange_RequestTree: function ()
	{
		...
		{
			//com.xiaonei.utils.dumpall("curr-request", currentRequest);
			//alert("avail.? ");
			if (currentRequest.isContentAvailable())
			{
				// async
	...
	showRawContent: function (status)
	{
		...
		if (com.xiaonei.utils.isContentTypeXml(currentRequest.ContentType))
		{
			// enable pretty
	...
	showRawContentError: function (status)
	{
		...
		document.getElementById("hf_RawContentOutput").value = this.stringBundle.GetStringFromName("overlay.requestdetails.contenttab.raw.error") + com.xiaonei.utils.nsResultErrors[status.toString(16)] + ")";
	},
	...
	showQueryStringParameters: function (request)
	{
		...
		{
			this.addHeaderRow("hf_QueryStringChildren", com.xiaonei.utils.urlDecode(request.QueryStringParameters[i][0]), com.xiaonei.utils.urlDecode(request.QueryStringParameters[i][1]));
		}
	},
	...
	showCookieInfo: function (request)
	{
		...
		{
			this.addCookieRow("hf_CookiesSentChildren", request.CookiesSent[i]["name"], request.CookiesSent[i]["value"], (request.CookiesSent[i]["path"]) ? request.CookiesSent[i]["path"] : "", (request.CookiesSent[i]["domain"]) ? request.CookiesSent[i]["domain"] : "", (request.CookiesSent[i]["expires"]) ? com.xiaonei.utils.formatDateTime(request.CookiesSent[i]["expires"]) : "End Of Session");
		}
	...
	showPostData: function (request)
	{
		...
		else if (request.PostDataParameters)
		{
			...
			for (i in request.PostDataParameters)
			{
				...
				{
					this.addHeaderRow("hf_PostDataChildren", com.xiaonei.utils.urlDecode(request.PostDataParameters[i][0]), com.xiaonei.utils.urlDecode(request.PostDataParameters[i][1] != null ? request.PostDataParameters[i][1] : ""));
				}
			}
		}
		else
		{
			...
			if (com.xiaonei.utils.isXml(request.PostData))
			{
	...
	getDebugInfoContent: function (request)
	{
		...
		content += "<b>Loadflags: - request:</b> " + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.LOADFLAGS_REQUEST, request.LoadFlags) + " (" + request.LoadFlags + ") <br/>";
		content += "<b>Loadflags: - channel:</b> " + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.LOADFLAGS_CHANNEL, request.LoadFlags) + " (" + request.LoadFlags + ") <br/>";
		content += "<b>Loadflags: - caching:</b> " + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.LOADFLAGS_CACHING, request.LoadFlags) + " (" + request.LoadFlags + ") <br/>";
		...
		for (i in request.RequestLog)
		{
			...
			content += "<b> - FoxMeter starttime: </b>" + request.FoxMeter.StartTime + "<br/>";
			//content += "- relative timestamp: " + com.xiaonei.utils.formatTime(new Date(this.RequestLog[i].Timestamp.getTime() - this.FoxMeter.StartTime.getTime())) + "<br/>";
			...
			content += "<b> - EventSource: </b>" + this.FoxMeterService.getEventSourceName(request.RequestLog[i].EventSource) + "<br/>";
			content += "<b> - EventSourceData: </b><br/>";
			for (u in request.RequestLog[i].EventSourceData)
			{
				if ((request.RequestLog[i].EventSource == this.FoxMeterService.FoxMeterEventSourceType.EVENTSINK_ON_STATUS ||
					request.RequestLog[i].EventSource == this.FoxMeterService.FoxMeterEventSourceType.WEBPROGRESS_ON_STATUS_CHANGED)
					&& u == "status")
				{
					content += "<b> -- " + u + ": </b>" + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.SOCKETTRANSPORT, request.RequestLog[i].EventSourceData[u]) + "<br/>";
				}
				else if (request.RequestLog[i].EventSource == this.FoxMeterService.FoxMeterEventSourceType.WEBPROGRESS_ON_SECURITY_CHANGED && u == "state")
				{
					content += "<b> -- " + u + " - security: </b>" + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.WEBPROGRESS_SECURITY, request.RequestLog[i].EventSourceData[u]) + "<br/>";
					content += "<b> -- " + u + " - strength: </b>" + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.WEBPROGRESS_SECURITY_STRENGTH, request.RequestLog[i].EventSourceData[u]) + "<br/>";
				}
				else if (request.RequestLog[i].EventSource == this.FoxMeterService.FoxMeterEventSourceType.WEBPROGRESS_ON_STATE_CHANGED && u == "flags")
				{
					content += "<b> -- " + u + " - transition: </b>" + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.WEBPROGRESS_TRANSITION, request.RequestLog[i].EventSourceData[u]) + "<br/>";
					content += "<b> -- " + u + " - type: </b>" + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.WEBPROGRESS_TYPE, request.RequestLog[i].EventSourceData[u]) + "<br/>";
					content += "<b> -- " + u + " - modifier: </b>" + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.WEBPROGRESS_MODIFIER, request.RequestLog[i].EventSourceData[u]) + "<br/>";
					content += "<b> -- " + u + " - security: </b>" + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.WEBPROGRESS_SECURITY, request.RequestLog[i].EventSourceData[u]) + "<br/>";
					content += "<b> -- " + u + " - strength: </b>" + this.FoxMeterService.getStatusTextFromCode(this.FoxMeterService.FoxMeterStatusCodeType.WEBPROGRESS_SECURITY_STRENGTH, request.RequestLog[i].EventSourceData[u]) + "<br/>";
				}
				else if (request.RequestLog[i].EventSource == this.FoxMeterService.FoxMeterEventSourceType.WEBPROGRESS_ON_LOCATION_CHANGED && u == "uri")
				{
	...
	Clipboard_CopyTreeRowCell: function ()
	{
		...
		{
			com.xiaonei.utils.toClipboard(this.currentCellText);
		}
	},
	...
	Clipboard_CopyTreeRow: function ()
	{
		if (this.currentContextTree != null)
		{
			...
			com.xiaonei.utils.toClipboard(copyString);
		}
	},
	...
	{
		com.xiaonei.utils.openWindow("foxmeter", "chrome://foxmeter/content/foxmeterwindow.xul", "", null);
	},

	OpenOptions: function ()
	{
		openDialog("chrome://foxmeter/content/foxmeteroptions.xul", "FoxMeter Options", "", null);
	}

}

var FoxMeter = new FoxMeterController();

function shutdownFoxMeter()
{
	FoxMeter.clear();
	//FoxMeter.cmd_stopWatching();

	FoxMeter.FoxMeterService.removeController(FoxMeter);

	if (FoxMeter.WindowMode)
	{
		FoxMeter.FoxMeterService.FoxMeterWindow = null;
		FoxMeter.FoxMeterService.windowIsClosed();
	}
